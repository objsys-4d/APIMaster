...
$function_t:="ZipCodeLookupRequest"
$data_o:=$1
$request_t:=USPSI_BuildConnectionString ("ShippingAPI.dll";"ZipCodeLookup";$function_t)
$data_t:=USPSI_PrepareData ($data_o;"Address")
If (OB Is empty:C1297(uspsErrs_o))
	$request_t:=$request_t+$data_t+"</"+$function_t+">"
	  //SET TEXT TO PASTEBOARD($request_t)
	  //TRACE
